...
export default function ToolDetailClient({ initialData }) {
  const { user } = useAuth();
  ...
  const load = () => fetch(`/api/tools/${toolId}`).then(r => r.json()).then(setData).catch(() => {});

  const addComment = async (e) => {
    e.preventDefault();
    if (!user) return router.push('/login');
    const r = await fetch(`/api/tools/${toolId}/comments`, {
