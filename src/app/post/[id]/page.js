...
export default function PostDetailPage() {
  ...
  const load = () => fetch(`/api/posts/${id}`).then(r => r.json()).then(d => setPost(d.post)).catch(() => {}).finally(() => setLoading(false));
  ...
  if (loading) return <div className="max-w-2xl mx-auto px-4 py-8"><div className="animate-pulse space-y-4"><div className="h-8 bg-bg-2 rounded w-1/3" /><div className="h-48 bg-bg-2 rounded-xl" /></div></div>;
