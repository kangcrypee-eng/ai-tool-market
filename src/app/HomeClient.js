...
export default function HomeClient({ initialPosts = [], initialTools = [] }) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    if (mode === 'contest') {
      ...
      if (!initialized && initialTools.length > 0) { setInitialized(true); return; }
      setLoading(true);
      loadTools().catch(() => setLoadError('툴 목록을 불러오지 못했습니다.')).finally(() => setLoading(false));
    } else {
      if (!initialized && initialPosts.length > 0) { setInitialized(true); return; }
      setLoading(true);
